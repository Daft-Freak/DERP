...
void DMA::update(uint64_t target)
{
    ...
    for(uint32_t cycle = 0; cycle < passed; cycle++)
    {
        ...
        if(!transferDataFifo.empty() && curWriteChannel >= 0)
        {
            ...
            if(transferSize == 1)
                mem.write<uint8_t>(this, curWriteAddr, val, cycles, false);
            else if(transferSize == 2)
                mem.write<uint16_t>(this, curWriteAddr, val, cycles, false);
            else
                mem.write<uint32_t>(this, curWriteAddr, val, cycles, false);
        ...
        if(!transferDataFifo.full() && curReadChannel >= 0)
        {
            ...
            if(transferSize == 1)
                val = mem.read<uint8_t>(this, curReadAddr, cycles, false);
            else if(transferSize == 2)
            {
                val = mem.read<uint16_t>(this, curReadAddr, cycles, false);
                if(bswap)
                    val = val >> 8 | val << 8;
            }
            else
            {
                val = mem.read<uint32_t>(this, curReadAddr, cycles, false);
                if(bswap)
